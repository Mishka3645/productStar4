...
public class Matrices
{
    ...
    public static class Vector
    {
        ...
        public Vector add(Vector other) {
            Vector sumVector = new Vector(items.length);
            for(int i = 0; i < items.length; ++i)
            {
                sumVector.items[i] = items[i] + other.items[i];
            }
            return sumVector;
        }
        ...
        public Vector subtract(Vector other) {
            Vector subtractVector = new Vector(items.length);
            for(int i = 0; i < items.length; ++i)
            {
                subtractVector.items[i] = items[i] - other.items[i];
            }
        ...
        public int dotProduct(Vector other) {
            ...
            for(int i = 0; i < items.length; ++i)
            {
                int temp = items[i] * other.items[i];
                dotProduct += temp;
            }
            return dotProduct;
        }
        ...
        public Vector scalarMultiply(int scalar) {
            Vector  scalarMultiply = new Vector(items.length);
            for(int i = 0; i < items.length; ++i)
            {
                scalarMultiply.items[i] = items[i] * scalar;
            }
    ...
    public static class Matrix
    {
        ...
        public Matrix add(Matrix other) {
            Matrix add = new Matrix(nRows, nCols); //rows - строки, cols - столбы
            for(int iRows = 0; iRows < nRows; ++iRows)
            {
                for(int iCols = 0; iCols < nCols; ++iCols)
                {
                    add.rows[iRows][iCols] = rows[iRows][iCols] + other.rows[iRows][iCols];
                }
            }
            return add;
        }
        ...
        public Matrix subtract(Matrix other) {
            Matrix subtract = new Matrix(nRows, nCols); //rows - строки, cols - столбы
            for(int iRows = 0; iRows < nRows; ++iRows)
            {
                for(int iCols = 0; iCols < nCols; ++iCols)
                {
                    subtract.rows[iRows][iCols] = rows[iRows][iCols] - other.rows[iRows][iCols];
                }
            }
            return subtract;
        }
        ...
        public Matrix multiply(Matrix other) {
            Matrix multiply = new Matrix(nRows, nCols);
            for(int i = 0; i < other.nRows; ++i)
            {
                for(int j = 0; j < other.nCols; ++j)
                {
                    for(int iRows = 0; iRows < other.nRows; ++iRows)
                    {
                        multiply.rows[j][i] += other.rows[iRows][i] * rows[j][iRows];
                    }
                }
            }
            return multiply;
        ...
        {
            Matrix scalarMultiply = new Matrix(nRows, nCols);
            for(int iRows = 0; iRows < nRows; ++iRows)
            {
                for(int iCols = 0; iCols < nCols; ++iCols)
                {
                    scalarMultiply.rows[iRows][iCols] = rows[iRows][iCols] * scalar;
                }
            }
            return scalarMultiply;
        }
        ...
        {
                Matrix transpose = new Matrix(nRows, nRows);
            for(int iRows = 0; iRows < nRows; ++iRows)
            {
                for(int jCols = iRows; jCols < nCols; ++jCols)
                {
                    transpose.rows[iRows][jCols] = rows[jCols][iRows];
                    transpose.rows[jCols][iRows] = rows[iRows][jCols];
                }
            }
            return transpose;
        }
        ...
        {
            int result = rows[0][0] * rows[1][1] * rows[2][2] +
        ...
        static int determinant(int[][] matrix) {
            ...
            for (int i = 0; i < matrix.length; i++) {
                ...
                for(int iRows = 1; iRows < matrix.length; ++iRows)
                {
                    for( int jCols = 0, zCols = 0; jCols < matrix.length; ++jCols)
                    {
                        if(jCols == i)
                            continue;
                        smallerMatrix[iRows - 1][zCols] = matrix[iRows][jCols];
                        ++zCols;
                    }
                }
    ...
    public static void main(String[] args) {
        ...
        System.out.println("Determinant of Matrix H second method:");
        System.out.println(Matrix.determinant(H.rows));
    }
}